  ...
  steps = [
    {
      ...
      args = [
        "build",
        "-t",
        "${var.region}-docker.pkg.dev/${var.project_id}/${var.project_id}/${local.pipeline_backend_repo_name}:$SHORT_SHA",
        ".",
      ]
    },
    {
      ...
      args = [
        "tag",
        "${var.region}-docker.pkg.dev/${var.project_id}/${var.project_id}/${local.pipeline_backend_repo_name}:$SHORT_SHA",
        "${var.region}-docker.pkg.dev/${var.project_id}/${var.project_id}/${local.pipeline_backend_repo_name}:latest",
      ]
    },
    {
      id   = "push-sha"
